...
class ProfileController extends GetxController {
  ...
  late var accountId;
  late final apiInfo = ApiInfo();

  var fav1 = 'https://api.themoviedb.org/3/account/\$accountId';
  var fav2 =
      '/favorite/movies?api_key=c6bd8982f6e8aaa3c24cef0599a47755&session_id=';
  var fav3 = '&language=en-US&sort_by=created_at.asc&page=1';

  Future<void> fetchProfileData() async {
    print('${sessionId}');
    final response =
        await http.get(Uri.parse(adr1 + apiInfo.api_key + adr2 + sessionId));

    if (response.statusCode == 200) {
      ProfileModel _playing = ProfileModel.fromJson(jsonDecode(response.body));

      this.accountId = _playing.id;
      print('acc id fetch:' + '${accountId}');

      playingList.add(ProfileModel(
          avatar: _playing.avatar,
          id: _playing.id,
          includeAdult: _playing.includeAdult,
          username: _playing.username));

      print('avatarpathfetch:' + '${playingList[0].avatar.tmdb.avatarPath}');
